...
const StackInputGroup = (props) =>{
    const {type, as, rows, bsPrefix, disabled, value, rounded, children, size, labelBg, icon, placeholder,id, labelText, labelInline} = props;
    return(
    ...
    <InputGroup.Text id={id}>{labelText}</InputGroup.Text>
                ...
                : <><Form.Label>{labelText}</Form.Label> <Form.Control as={as} rows={rows} bsPrefix={bsPrefix} size={size} {...props} /> </>}
            </StyledInputGroup>
